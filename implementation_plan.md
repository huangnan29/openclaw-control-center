...
当前阶段推进 **Tom 单 Oracle 本机多实例上线** 与 **人工批准的只读 healthcheck live 演练**：中央 control-center 已完成 registry、collector snapshot 读取、本地 exporter、Tom collector cron、本机最终上线状态汇总入口、最终上线总闸门、本机最终上线推进 runner、本机实例安全注册脚本、dry-run API、dry-run UI、OpenClaw/Discord 可调用的管理动作 dry-run 命令入口与文本指令解析、dry-run 证据闸门、人工确认字段、审计检索视图、默认关闭的 live gate、测试用 mock executor、真实执行审计结果类型、dry-run 引用校验、灰度配置解析、rollout 决策响应、只读 readiness 卡片、生产执行器最小骨架、显式挂载开关、只读 healthcheck 人工演练材料、Tom preflight、一次性演练窗口脚本、实例影响快照留证、结构化 approval 记录、approval 准备/状态查看、批准前证据包生成与校验、显式 approval `approve` 记录命令、approval `approve` 前强制证据包校验、只读 live healthcheck readiness 汇总、自动推进到人工批准前和人工批准后的 live healthcheck rollout runner、final runner 重复 `prepare` 幂等返回人工批准/已批准边界、approval 一次性 `consume` 状态和演练报告生成。`OPENCLAW_TOPOLOGY_MODE` 默认 `local-only`，因此第二台 Oracle host/key 不再阻塞当前上线；同一台 Oracle 新增实例时，使用 `register-local-instance.sh plan/apply` 只更新 control-center registry 与 compose 只读挂载，再刷新 collector snapshot 和 healthcheck；如果未来扩展到其他 Oracle，再显式设置 `OPENCLAW_TOPOLOGY_MODE=cross-server` 并使用已保留的远端凭据发现、onboarding、preflight、pull、register 和 rollout runner 链路。Tom 仍不默认启用 live gate 或 executor；下一步可用 `final-go-live-runner.sh prepare` 从本机总入口自动执行最终 `check`、Tom runner prepare 和复核，停在人工批准前；如果已经到达人工批准边界，重复执行 `prepare` 会直接返回当前边界，不再次写 Tom runtime；OpenClaw/Discord 可以先通过 `managed-action-command-runner.sh parse-text/plan-text/dry-run-text` 把“对 tom 运行 zhihu-human-ops-writing dry-run”解析为受控 payload 并预览 `healthcheck`、`collector_refresh`、`skill_run`，该入口要求文本明确包含 dry-run/预览/演练并拒绝 live、发布、重启、approval 等高风险词，只调用 dry-run API 生成审计，不执行实例命令，并可在显式 `MANAGED_ACTION_COMMAND_TOKEN_SOURCE=container` 时从 control-center 容器读取本地令牌而不打印；readiness、dry-run gate 和总闸门输出的下一步都已收敛到 runner 主链路；人工 approval 已批准后，`final-go-live-runner.sh run-approved` 或 Tom `live-healthcheck-rollout-runner.sh run-approved` 会再次确认 readiness、要求本地令牌与显式 runner 确认，然后执行一次 `healthcheck` live 演练；前置条件或人工批准未满足时 runner 返回非 0，避免自动化误判阻断态为成功。
