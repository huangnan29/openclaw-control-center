...
人工批准 live healthcheck 前，推荐先让 runner 自动准备批准前证据包并停在人工批准边界：
...
`status` 只读检查 Tom 容器 token 长度和 approval review 状态；它不打印 token、不写 approval、不打开 live gate。当前 ready 时会返回 `preflight_ready_for_human_approval`。
