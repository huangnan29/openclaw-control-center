...
当前阶段推进 **Tom 单 Oracle 本机多实例上线** 与 **人工批准的只读 healthcheck live 演练**：中央 control-center 已完成 registry、collector snapshot 读取、本地 exporter、Tom collector cron、本机最终上线状态汇总入口、最终上线总闸门、本机实例安全注册脚本、dry-run API、dry-run UI、dry-run 证据闸门、人工确认字段、审计检索视图、默认关闭的 live gate、测试用 mock executor、真实执行审计结果类型、dry-run 引用校验、灰度配置解析、rollout 决策响应、只读 readiness 卡片、生产执行器最小骨架、显式挂载开关、只读 healthcheck 人工演练材料、Tom preflight、一次性演练窗口脚本、实例影响快照留证、结构化 approval 记录、approval 准备/状态查看、批准前证据包生成与校验、显式 approval `approve` 记录命令、approval `approve` 前强制证据包校验、只读 live healthcheck readiness 汇总、自动推进到人工批准前和人工批准后的 live healthcheck rollout runner、approval 一次性 `consume` 状态和演练报告生成。`OPENCLAW_TOPOLOGY_MODE` 默认 `local-only`，因此第二台 Oracle host/key 不再阻塞当前上线；同一台 Oracle 新增实例时，使用 `register-local-instance.sh plan/apply` 只更新 control-center registry 与 compose 只读挂载，再刷新 collector snapshot 和 healthcheck；如果未来扩展到其他 Oracle，再显式设置 `OPENCLAW_TOPOLOGY_MODE=cross-server` 并使用已保留的远端凭据发现、onboarding、preflight、pull、register 和 rollout runner 链路。Tom 仍不默认启用 live gate 或 executor；下一步用 `final-go-live-status.sh status/check` 汇总 Tom 本体健康、当前拓扑、dry-run 证据和 live 管理动作 readiness；管理动作侧先用 `managed-action-dry-run-gate.sh status/run` 确保 dry-run 审计证据有效，或用 `live-healthcheck-rollout-runner.sh prepare` 自动准备 approval 模板、生成并校验证据包、刷新 readiness 后停在人工批准前；readiness、dry-run gate 和总闸门输出的下一步都已收敛到 runner 主链路；人工 approval 已批准后，`live-healthcheck-rollout-runner.sh run-approved` 会再次确认 readiness、要求本地令牌与显式 runner 确认，然后执行一次 `healthcheck` live 演练；前置条件或人工批准未满足时 runner 返回非 0，避免自动化误判阻断态为成功。
