...
当前阶段推进 **只读 live readiness 诊断卡部署验证**：中央 control-center 已完成 dry-run API、dry-run UI、人工确认字段、审计检索视图、默认关闭的 live gate、测试用 mock executor、真实执行审计结果类型、dry-run 引用校验、灰度配置解析、rollout 决策响应和只读 readiness 卡片。当前准备部署到 Tom 验证行为不变；Tom 不接入执行器、不开启真实执行、不调用 OpenClaw 实例命令。
