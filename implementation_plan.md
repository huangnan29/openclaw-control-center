...
当前阶段推进 **受控管理动作 dry-run**：中央 control-center 已在 Tom 上完成 5 个实例的只读 collector 聚合和页面新鲜度展示。Tom 之外的候选 Oracle 主机暂不能安全 SSH 接入，因此先进入不影响实例的管理动作预览层：只列出白名单动作、生成命令预览、写入控制中心审计日志，并在总览页提供 dry-run UI，不执行任何 OpenClaw 实例命令。
