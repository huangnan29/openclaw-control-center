...
当前阶段推进 **受控管理动作 dry-run**：中央 control-center 已在 Tom 上完成 5 个实例的只读 collector 聚合和页面新鲜度展示。Tom 之外的候选 Oracle 主机暂不能安全 SSH 接入，因此先进入不影响实例的管理动作预览层：只列出白名单动作、生成命令预览、要求操作者与确认短语、写入控制中心审计日志，并在总览页提供 dry-run UI 和审计检索视图，不执行任何 OpenClaw 实例命令。
...
5. 白名单真实执行设计：先实现配置和测试，默认关闭，不在 Tom 执行真实动作。
6. 白名单真实执行灰度：默认关闭，只在单实例、单动作、人工确认下开启。
7. 扩展跨服务器：等第二台 Oracle 能安全接入后，再按 collector 只读方式加入中央视图。
