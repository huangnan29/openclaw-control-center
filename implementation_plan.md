...
当前阶段推进 **Tom 单 Oracle 本机多实例上线** 与 **人工批准的只读 healthcheck live 演练**：中央 control-center 已完成 registry、collector snapshot 读取、本地 exporter、Tom collector cron、本机最终上线状态汇总入口、最终上线总闸门、本机最终上线推进 runner、本机最终上线 approval token 包装器及只读 preflight、本机最终上线人工审查摘要、本机最终上线完成度审计、本机实例安全注册脚本、dry-run API、dry-run UI、OpenClaw/Discord 可调用的管理动作 dry-run 命令入口、文本指令解析、机器人文本桥接层、workspace inbox runner 和 Tom `AGENTS.md` inbox 规范安装器、dry-run 证据闸门、heartbeat/token 异常用量只读检查器、异常用量告警 runner 与受控 cron 安装器、人工确认字段、审计检索视图、默认关闭的 live gate、测试用 mock executor、真实执行审计结果类型、dry-run 引用校验、灰度配置解析、rollout 决策响应、只读 readiness 卡片、生产执行器最小骨架、显式挂载开关、只读 healthcheck 人工演练材料、Tom preflight、一次性演练窗口脚本、实例影响快照留证、结构化 approval 记录、approval 准备/状态查看、批准前证据包生成与校验、批准前只读 approval review 汇总、显式 approval `approve` 记录命令、approval `approve` 前强制证据包校验、只读 live healthcheck readiness 汇总、自动推进到人工批准前和人工批准后的 live healthcheck rollout runner、final runner 重复 `prepare` 幂等返回人工批准/已批准边界、approval 一次性 `consume` 状态和演练报告生成、显式确认的 `approve-and-run` 本机总入口、演练后 `verify-completed` 只读验收入口。`OPENCLAW_TOPOLOGY_MODE` 默认 `local-only`，因此第二台 Oracle host/key 不再阻塞当前上线；同一台 Oracle 新增实例时，使用 `register-local-instance.sh plan/apply` 只更新 control-center registry 与 compose 只读挂载，再刷新 collector snapshot 和 healthcheck；如果未来扩展到其他 Oracle，再显式设置 `OPENCLAW_TOPOLOGY_MODE=cross-server` 并使用已保留的远端凭据发现、onboarding、preflight、pull、register 和 rollout runner 链路。Tom 仍不默认启用 live gate 或 executor；下一步可用 `final-go-live-runner.sh prepare` 从本机总入口自动执行最终 `check`、Tom runner prepare 和复核，停在人工批准前；如果已经到达人工批准边界，重复执行 `prepare` 会直接返回当前边界，不再次写 Tom runtime，并且会只读执行 approval review，把 `approve-and-run` 单命令作为下一步；OpenClaw/Discord 可以先让 Tom 在 workspace 的 `control-center-commands/inbox/*.txt` 写入“对 tom 运行 zhihu-human-ops-writing dry-run”，再由 `managed-action-inbox-runner.sh status/plan-next/run-next` 通过只读挂载读取 inbox、调用 `managed-action-text-bridge.sh`，预览 `healthcheck`、`collector_refresh`、`skill_run`；`install-managed-action-agents-instructions.sh` 只把这条行为规范安装进 Tom workspace 的 `AGENTS.md` 受控标记块，并备份原文件，不修改 `openclaw.json`、不重启实例；异常用量 cron 只调用 `heartbeat-burn-alert-runner.sh run`，把结果写入 control-center runtime，不清空 `HEARTBEAT.md`、不调用模型、不修改实例目录；`final-go-live-review.sh status` 只读汇总 Tom readiness、approval packet、approval、两个 cron 和最新 heartbeat 告警，给人工批准前提供短摘要，不批准、不写 runtime、不打开 live gate；`final-go-live-completion-audit.sh status` 只读运行 review 和 Tom healthcheck，把目标拆成 pass/warning/pending/fail，明确 heartbeat/token 告警是 warning、最终 live healthcheck 仍需人工批准，不把等待状态误判为完成；该入口要求文本明确包含 dry-run/预览/演练并拒绝 live、发布、重启、approval 等高风险词，只调用 dry-run API 生成审计，不执行实例命令，不移动或删除 OpenClaw workspace 文件，并可在显式 `MANAGED_ACTION_COMMAND_TOKEN_SOURCE=container` 时从 control-center 容器读取本地令牌而不打印；readiness、approval review、dry-run gate 和总闸门输出的下一步都已收敛到 runner 主链路；人工 approval 由 Anan 审查后，`final-go-live-runner.sh approve-and-run` 可以在强确认、批准人和本地令牌齐全时先只读复核 approval review，再记录 approval 并执行一次 `healthcheck` live 演练；如果不想手工导出令牌，可以先用 `final-go-live-approve-and-run-from-tom-token.sh status` 只读确认 Tom 容器 token 长度和 review ready，再在同样强确认下用 `approve-and-run` 从 Tom 容器环境读取令牌到子进程，不打印、不落盘，并委托既有 runner；也可以先手动 approval，再用 `final-go-live-runner.sh run-approved` 或 Tom `live-healthcheck-rollout-runner.sh run-approved` 执行；执行成功后会自动运行 `verify-completed`，确认 approval 已消费、最新报告通过、只读状态恢复且没有 OpenClaw 实例影响。前置条件、人工批准、approval review 或演练后验收未满足时 runner 返回非 0，避免自动化误判阻断态为成功。
...
补充异常用量告警状态：`heartbeat-burn-inspector.sh` 已能从 collector history 识别 deepseek 这类周期性小额 token 增长，并只读读取 `HEARTBEAT.md` 元数据；`heartbeat-burn-alert-runner.sh run` 会把最新结果写入 `runtime/heartbeat-burn-alerts/latest.json`，可疑时追加 `events.ndjson` 并返回非 0；`install-heartbeat-burn-alert-cron.sh` 负责安装 `OPENCLAW_HEARTBEAT_BURN_ALERT_CRON` 受控块。该链路只写 control-center runtime 与 crontab，不写 OpenClaw 实例目录、不清空 heartbeat、不调用模型、不重启实例。
