...
当前阶段并行推进 **跨服务器只读 collector 接入** 与 **人工批准的只读 healthcheck live 演练**：中央 control-center 已完成 registry、collector snapshot 读取、本地 exporter、Tom collector cron、本机远端 Oracle 只读凭据候选发现工具、本机 push 配置渲染、本机到 Tom 的远端凭据推送工具、Tom 端远端 collector 凭据准备工具、远端 collector 接入包生成器、接入包内置可选 Docker build context、接入包离线 verify、远端 SSH 只读 preflight、preflight 本地状态留痕、跨服务器只读接入 rollout gate、跨服务器只读接入自动编排器、最终上线总闸门、远端凭据就绪闸门、远端 collector-only 节点 bootstrap、远端 collector snapshot 只读拉取脚本、Tom registry 安全注册脚本、collector-only 远端实例解析、dry-run API、dry-run UI、人工确认字段、审计检索视图、默认关闭的 live gate、测试用 mock executor、真实执行审计结果类型、dry-run 引用校验、灰度配置解析、rollout 决策响应、只读 readiness 卡片、生产执行器最小骨架、显式挂载开关、只读 healthcheck 人工演练材料、Tom preflight、一次性演练窗口脚本、实例影响快照留证、结构化 approval 记录、approval 准备/状态查看、显式 approval `approve` 记录命令、approval 一次性 `consume` 状态和演练报告生成。Tom 仍不默认启用 live gate 或 executor；下一步是先用 `discover-remote-oracle-credentials.sh scan/probe` 找到第二台 Oracle 的真实 host/user/sshKey，再用 `render-push-config` 生成本机 push 配置，并通过本机侧 `push-remote-collector-credentials.sh` 或 Tom 端 `remote-collector-credentials.sh` 补齐凭据；随后在 Tom 使用 `remote-collector-rollout-runner.sh run` 自动推进已满足条件的只读 preflight、远端 collector snapshot pull、registry plan/apply 和 `healthcheck.sh` 验收；平时用 `go-live-gate.sh status/check` 汇总 Tom 本体健康、跨服务器只读阶段和 live 管理动作 readiness；管理动作侧只有在 approval 文件通过 `approve/check` 校验、且提供本地令牌后，才执行一次 `healthcheck` live 演练。
...
25. 跨服务器只读接入 rollout gate：读取 onboarding bundle、preflight 状态、pull 状态、snapshot 和 registry，输出 `needs_remote_preflight`、`needs_remote_collector_pull`、`needs_registry_register` 或 `ready_for_healthcheck`，不 SSH、不写 registry、不写远端文件。
26. 白名单真实执行灰度：默认关闭，只在单实例、单动作、人工确认下开启。
