...
把 `openclaw-control-center` 从 Tom 单机灰度只读面板，逐步推进为可监控多台 Oracle 服务器、多套 OpenClaw 实例的中央控制中心。推进顺序必须先保证数据可信，再扩展跨服务器采集，最后才考虑受控管理动作。

## 当前阶段

当前阶段推进 **跨服务器 registry（只读）**：先让配置模型能表达多台 Oracle 服务器及其下实例，并在 UI 中增加服务器维度筛选和健康汇总。仍不做实例启停、审批、发布、重启、任务派发等写操作。
